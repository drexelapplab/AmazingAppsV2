...
//:
//: A **function** is a series of code that can be used to repeat an action or process information. For instance, using the volume example.  We could write a function that checks the volume. Functions also use the parentheses *()* and curly brackets *{}* as part of their syntax.  You also have to declare it as a function using the term *func* (for Swift).
//:
//:  **Example:**
